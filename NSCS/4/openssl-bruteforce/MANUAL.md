...
Sending a USR1 signal to a running ./bruteforcer process makes
...
only passwords with 5 characters:
    ...
    ./bruteforcer -t 4 -l 5 -m 5 -c aes256 encrypted.file
...
and containing only letters:
    ...
    ./bruteforcer -t 8 -l 9 -m 11 -c des3 -b "AbCD" -e "Ef" \
...
the passwords contained in a dictionary file:
    ...
    ./bruteforcer -t 6 -f dictionary.txt -c aes256 encrypted-file


Print progress info every 30 seconds:

    ./bruteforcer -t 6 -f dictionary.txt -c aes256 -v 30 encrypted-file


Try to find the password of a des3 encrypted gzip file using 8 threads:

    ./bruteforcer -t 8 -v 5 -c des3 -m 9 -s "0123456789" -M "$(echo -ne '\x1f\x8b')" file.tar.gz.des3


Save/restore state between sessions:

    ./bruteforcer -t 6 -f dictionary.txt -c aes256 -w state.txt encrypted-file
      (Let the program run for a few minutes and stop it)
    ./bruteforcer -t 6 -c aes256 -w state.txt encrypted-file
